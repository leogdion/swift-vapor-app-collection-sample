...
final class AppleSoftwareProductController {
  ...
  func create(_ req: Request) throws -> Future<AppleSoftwareProductResponse> {
    ...
    return try futureClient.flatMap { (client) in
      ...
      return response.body.consumeData(on: req)
    }.map { (data)  in
      return try self.jsonDecoder.decode(AppleSearchResult.self, from: data)
      ...
      // find exists product
      AppleSoftwareProduct.query(on: req).filter(\.trackId == iTunesTrackID).first().map { (foundAppleSWProduct)  in
